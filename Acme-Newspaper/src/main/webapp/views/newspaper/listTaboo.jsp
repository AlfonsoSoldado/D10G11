...
<display:table pagesize="5" class="displaytag" keepStatus="true"
	...
	<security:authorize access="hasRole('ADMIN')">
		...
		<display:column>
			<acme:links url="newspaper/display.do?newspaperId=${row.id}"
		...
		<display:column>
			<img class="imagesNewspaper" src="${row.picture}">
		</display:column>

		<display:column>
			<acme:links url="article/list.do?newspaperId=${row.id}"
				code="newspaper.articles" />
		</display:column>
		<display:column>
			<acme:links url="user/display.do?userId=${row.publisher.id}"
				code="newspaper.publisher" />
		</display:column>
	</security:authorize>
